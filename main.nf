...
process bamToBed {
  ...
  input:
  file split_bam from split_bam_ch
  file disc_bam from disc_bam_ch
  file concordant_bam from concordant_bam_ch
  val sample_id from sample_id_ch5

  output:
  file "*.split.txt" into split_txt_ch, split_txt_ch2
  file "*.concordant.txt" into concordant_txt_ch, concordant_txt_ch2
  file "*.disc.txt" into disc_txt_ch
  val sample_id into sample_id_ch7
...
process readIdFrequency {
  publishDir "results/readIdFrequency", mode: "copy"

  input:
  file split_txt from split_txt_ch
  file concordant_txt from concordant_txt_ch
  val sample_id from sample_id_ch7

  output:
  file "*.split.id-freq.txt" into split_id_freq_ch
  file "*.split.id-freq2.txt" into split_id_freq2_ch
  file "*.split.id-freq4.txt" into split_id_freq4_ch

  file "*.concordant.id-freq.txt" into concordant_id_freq_ch
  file "*.concordant.id-freq3.txt" into concordant_id_freq3_ch
  file "*.concordant.id-freqGr3.txt" into concordant_id_freqGr3_ch

  val sample_id into sample_id_ch8

  script:
  """
  awk '{print \$4}' $split_txt | sort | uniq -c > "${sample_id}.split.id-freq.txt"
  ...
  awk '{print \$4}' $concordant_txt | sort | uniq -c > "${sample_id}.concordant.id-freq.txt"
...
process selectSplitReads {
  publishDir "results/splitReads", mode: "copy"
  echo true
  cache false

  input:
  //file split_id_freq from split_id_freq_ch
  file split_id_freq2 from split_id_freq2_ch
  file split_id_freq4 from split_id_freq4_ch

  //file concordant_id_freq from concordant_id_freq_ch
  file concordant_id_freq3 from concordant_id_freq3_ch
  file concordant_id_freqGr3 from concordant_id_freqGr3_ch

  file split_txt from split_txt_ch2
  file concordant_txt from concordant_txt_ch2

  val sample_id from sample_id_ch8
  ...
  val sample_id into sample_id_ch9
